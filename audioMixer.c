...
void AudioMixer_queueSound(wavedata_t *pSound)
{
    ...
    if ( freeSlotExist == false ){
        //! ASK: TOO SLOW, make the flow slower
        printf("ERROR: Unable to find free slot in sound bite\n");
        return;
    }
    return;
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	 ...
	 *    - Otherwise "add" this sound bite's data to the play-back buffer (other
    ...
    //! ASK: Doesn't work, sound iz weird
        ...
        if ( soundBites[i].pSound != NULL ){
            ...
            for ( int j = 0; j < size; j++ ) {
                if ( offset < soundBites[i].pSound->numSamples) {
                    int dataSound = (int)soundBites[i].pSound->pData[offset];
                    dataSound += playbackBuffer[j];
                    if ( dataSound > SHRT_MAX ) {
                        dataSound = SHRT_MAX;
                    }
                    else if ( dataSound < SHRT_MIN ) {
                       dataSound= SHRT_MIN;
                    }
                    playbackBuffer[j] = (short)dataSound;
                }
                else {
                    soundBites[i].pSound = NULL;
                    offset = 0;
